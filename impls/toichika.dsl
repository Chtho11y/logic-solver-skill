...
for p in cells():
    ...
    for q in cells():
        if row_of(p) == row_of(q) and col_of(p) < col_of(q):
            let mid = count_where(row(row_of(p)), fn (s) -> col_of(p) < col_of(s) and col_of(s) < col_of(q) and at(a, s) != 8)
        ...
        if row_of(p) == row_of(q) and col_of(q) < col_of(p):
            let mid = count_where(row(row_of(p)), fn (s) -> col_of(q) < col_of(s) and col_of(s) < col_of(p) and at(a, s) != 8)
            let ispair = b2i(at(a, q) == RIGHT and at(a, p) == LEFT and mid == 0)
            if regions_share_border(region_id(p), region_id(q)):
                ispair == 0
            let partners = partners + ispair
        if col_of(p) == col_of(q) and row_of(p) < row_of(q):
            let mid = count_where(col(col_of(p)), fn (s) -> row_of(p) < row_of(s) and row_of(s) < row_of(q) and at(a, s) != 8)
            let ispair = b2i(at(a, p) == DOWN and at(a, q) == UP and mid == 0)
            if regions_share_border(region_id(p), region_id(q)):
                ispair == 0
            let partners = partners + ispair
        if col_of(p) == col_of(q) and row_of(q) < row_of(p):
            let mid = count_where(col(col_of(p)), fn (s) -> row_of(q) < row_of(s) and row_of(s) < row_of(p) and at(a, s) != 8)
