...
- **中间**：vendored Penpa+（`web/public/penpa-edit/`）。画布是 Penpa 的 canvas，不是自家 SVG。求解时用 `pu.maketext()` 导出 URL，走 `/api/import` 绑定 DSL。
- **右侧**：`GET /api/puzzles/<key>` 的 `source`。求解把当前文本作为 `source` 交给 `/api/solve`。

未选择题型也可以画；导入 Penpa+ / puzz.link 写入画布。换题型不清空盘面。点「样例」才用该题 sample 覆盖。样例通过 `/api/encode` 编成 Penpa URL 再载入。
